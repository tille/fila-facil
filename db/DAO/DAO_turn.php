...
class DAO_turn {
  ...
  function DAO_get_user_by_turn($turn, $mod){
    $con = connect();
    $sql = "SELECT users.name, users.last_name, users.email, users.identification, users.Eafit_student, expected_turn.Info_Process FROM users INNER JOIN expected_turn ON users.identification=expected_turn.user_id AND expected_turn.module='$mod' AND expected_turn.expected_turn='$turn'";
    ...
    .$arr_res1['identification'].":".$arr_res1['Eafit_student'].":".$arr_res1['Info_Process'];
